...
class CrowdSimulation:
    ...
    def step(self) -> SimulationState:
        ...
        expected_attendance = getattr(self.venue, 'expected_attendance', None)
        remaining_spawns = expected_attendance - self.total_spawned if expected_attendance else float('inf')
        if remaining_spawns < 0:
            remaining_spawns = 0
            
        # Spawn initial burst at t=0
        if self.current_time == 0.0 and remaining_spawns > 0:
            new_agents = self.spawn_manager.spawn_initial_burst(self.current_time)
            if len(new_agents) > remaining_spawns:
                new_agents = new_agents[:int(remaining_spawns)]
            for agent in new_agents:
                self.agents[agent.id] = agent
            self.total_spawned += len(new_agents)
            remaining_spawns -= len(new_agents)
